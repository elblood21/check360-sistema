...
@section('title', 'Responder encuesta de salida')
                ...
                <div class="card-body p-3 text-white">
                    <div class="row align-items-center text-center text-md-start">
                        <div class="col-md-7">
                            ...
                            <p class="mb-0 opacity-75 small"><i class="fa fa-calendar me-1"></i> {{ $visita->fecha_asignacion ? $visita->fecha_asignacion->format('d/m/Y') : 'N/A' }} | <i class="fa fa-clock-o me-1"></i> {{ $visita->hora_asignacion ? date('H:i', strtotime($visita->hora_asignacion)) : 'N/A' }}</p>
                        </div>
                        <div class="col-md-5 text-md-end mt-2 mt-md-0">
                            <span class="badge bg-white text-primary px-3 py-2" style="border-radius: 8px;">Post-Visita</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body p-4">
                    ...
                    <form id="formEncuesta">
                        ...
                        <div class="step-section {{ $idx == 0 ? 'active' : '' }}" data-step="{{ $idx }}">
                            ...
                            <div class="text-center mb-4">
                                ...
                                <h4 class="fw-bold">{{ $idx == 0 ? 'Evaluación de ' . $dim : 'Continuamos con ' . $dim }}</h4>
                            </div>
                    ...
                    @else
                        ...
                        <div class="text-center">
                            <a href="{{route('visitas.ver', encrypt($visita->id))}}" class="btn btn-secondary">Volver</a>
                        </div>
                    @endif
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        if (totalSteps > 0) {
            ...
            // Focus visual en textarea/input text (para simular el click)
            $('.respuesta-pregunta').on('input focus', function() {
                const pId = $(this).data('pregunta-id');
                $(`#q-${pId}`).removeClass('error-shake');
                if ($(this).val().trim() !== '') {
                     $(`#q-${pId}`).addClass('active-focus');
                } else {
                     $(`#q-${pId}`).removeClass('active-focus');
                }
            });

            // Focus visual en preguntas (clicks generales en el box)
            ...
            $('#btnFinalize').click(function() {
                ...
                $.ajax({
                    url: '{{route("visitas.guardar_salida")}}',
                    method: 'POST',
                    data: {
